...
- 
：要获取的聊天数量，可取值 1 到 20。
- sort_order：可选排序方式，默认是 desc，表示按时间倒序（最新在前）；sc 表示正序（最早在前）。
- efore：可选时间过滤条件，用于获取在该时间之前更新的聊天（ISO 格式）。
- fter：可选时间过滤条件，用于获取在该时间之后更新的聊天（ISO 格式）。

**参数选择：**
- 可以组合使用 efore 与 fter，以获取特定时间范围内的聊天记录。
- 应根据目标策略性设置 
；如果希望尽量多拿信息，可设置为 
=20。
...
3. 同时提到时间和主题？→ 如果时间范围明确，使用 ecent_chats。否则，如果有两个及以上实质关键词，就用 conversation_search；否则仍用 ecent_chats。
4. 指代模糊？→ 先澄清
5. 没有历史指向？→ 不要使用这些工具
